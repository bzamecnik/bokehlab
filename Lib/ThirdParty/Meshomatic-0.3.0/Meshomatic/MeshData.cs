    ...
    public class MeshData
    {
        ...
        */
        ...
        public void OpenGLArrays(out float[] verts, out float[] norms, out float[] texcoords, out uint[] indices)
        {
            ...
            {
                Point p = points[i];
                verts[i * 3] = Vertices[p.Vertex].X;
                verts[i * 3 + 1] = Vertices[p.Vertex].Y;
                verts[i * 3 + 2] = Vertices[p.Vertex].Z;

                norms[i * 3] = Normals[p.Normal].X;
                norms[i * 3 + 1] = Normals[p.Normal].Y;
                norms[i * 3 + 2] = Normals[p.Normal].Z;

                texcoords[i * 2] = TexCoords[p.TexCoord].X;
                texcoords[i * 2 + 1] = TexCoords[p.TexCoord].Y;
